...
void loop()
{
    // Vérifie si une connexion Bluetooth est établie
    if (SerialBT.connected())
    {
        // Jouer un son pour indiquer que la connexion Bluetooth est établie
        if (buzzer_ON)
        {
    ...
    {
        // La connexion Bluetooth n'est pas établie, essayez de vous connecter au maître
        SerialBT.connect("ESP32_MASTER");

        // Définir la couleur des LED en rouge pour indiquer que la connexion est rompue ou en cours de connexion
